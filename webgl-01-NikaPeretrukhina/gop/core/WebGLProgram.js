...
layout(location=0) in vec3 position; // Eingabeattribut erwartet ein 3D-Vektor (vec3) als Eingabe und wird an Location 0 im Vertex-Shader gebunden
layout(location=1) in vec3 color; //Dieses Attribut erwartet ebenfalls einen 3D-Vektor (vec3) als Eingabe und wird an Location 1 im Vertex-Shader gebunden.
uniform mat4 modelMatrix;

out vec3 v_color;

void main() {
/*Diese Zeile berechnet die Position des aktuellen Vertex im Raumkoordinatensystem,
nachdem die Modelltransformation (Verschiebung, Rotation, Skalierung) angewendet wurde.*/
  
	 gl_Position = modelMatrix * vec4( position, 1.0 ); 
  v_color = color;//zuweisung
}
`;
